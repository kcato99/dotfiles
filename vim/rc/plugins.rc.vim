"-------------------------------------------------------------------------------
" プラグインごとの設定 Plugins
"-------------------------------------------------------------------------------

source ~/dotfiles/vim/rc/plugins/cascading.rc.vim
source ~/dotfiles/vim/rc/plugins/nerdtree.rc.vim
source ~/dotfiles/vim/rc/plugins/fugitive.rc.vim
source ~/dotfiles/vim/rc/plugins/nerdcommenter.rc.vim
source ~/dotfiles/vim/rc/plugins/tabular.rc.vim
source ~/dotfiles/vim/rc/plugins/camelcasemotion.rc.vim
source ~/dotfiles/vim/rc/plugins/fzf.rc.vim
source ~/dotfiles/vim/rc/plugins/quickrun.rc.vim
source ~/dotfiles/vim/rc/plugins/open-browser.rc.vim
source ~/dotfiles/vim/rc/plugins/minibufexpl.rc.vim
source ~/dotfiles/vim/rc/plugins/tagbar.rc.vim
source ~/dotfiles/vim/rc/plugins/lsp.rc.vim
" source ~/dotfiles/vim/rc/plugins/vim-go.rc.vim
" source ~/dotfiles/vim/rc/plugins/coc.rc.vim
source ~/dotfiles/vim/rc/plugins/airline.rc.vim
" source ~/dotfiles/vim/rc/plugins/defx.rc.vim
